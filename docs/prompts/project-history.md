...
- Rapprochement du zoom lors de la s̩lection d'un v̩hicule sur la carte Google afin de mieux cadrer le traceur sans perdre le contexte de rue.
- All̬gement visuel de la queue de trace derri̬re le v̩hicule en mouvement avec une opacit̩ plus faible et une ligne l̩g̬rement plus fine.
- Suppression des doublons Odom̬tre et Heures moteur dans le bloc Diagnostic traceur : ces informations restent centralis̩es dans le bloc OBD / CAN bus.
- Am̩lioration de la lecture OBD/CAN dans la fiche traceur : les valeurs peuvent d̩sormais ̻tre relues depuis les blocs `obd`, `can`, `io`, `sensors`, `payload` et les cl̩s num̩riques re̤ues dans le payload brut.
