...
<div class="modal fade users-modal maintenance-form-modal" id="maintenanceModal" tabindex="-1" aria-labelledby="maintenanceModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered users-modal-dialog maintenance-modal-dialog">
        <div class="modal-content">
            <form class="maintenance-modal-form" method="POST" action="{{ route('maintenance.store') }}" enctype="multipart/form-data" data-maintenance-form data-loading-form>
                @csrf
                ...
                <div class="modal-body maintenance-modal-body">
                    <section class="maintenance-form-section">
                        ...
                        <div class="users-form-grid">
                            ...
                            <div><label class="form-label">{{ __('maintenance.garage') }}</label><select class="form-select" name="garage_id" data-field="garage_id" data-searchable-database data-search-placeholder="{{ __('ui.search_options') }}" data-no-results="{{ __('ui.no_option_match') }}" data-option-icon="fa-screwdriver-wrench"><option value="">-</option>@foreach($garages as $garage)<option value="{{ $garage->id }}">{{ $garage->name }}</option>@endforeach</select></div>
                            <div class="users-form-full"><label class="form-label">{{ __('maintenance.name') }} *</label><input class="form-control" name="name" required data-field="name"></div>
                            <div><label class="form-label">{{ __('maintenance.type') }}</label><select class="form-select" name="maintenance_type" data-field="maintenance_type"><option value="preventive">{{ __('maintenance.preventive') }}</option><option value="corrective">{{ __('maintenance.corrective') }}</option></select></div>
                            <div><label class="form-label">{{ __('maintenance.estimated_cost') }}</label><div class="maintenance-input-suffix"><input class="form-control" type="number" min="0" step="0.01" name="estimated_cost" data-field="estimated_cost"><span>USD</span></div></div>
                            <div class="users-form-full"><label class="form-label">{{ __('maintenance.description') }}</label><textarea class="form-control" name="description" rows="3" data-field="description"></textarea></div>
                        </div>
                    </section>
                    <section class="maintenance-form-section">
                        <div class="maintenance-section-header"><i class="fa-solid fa-arrows-rotate"></i><h3>{{ __('maintenance.conditions') }}</h3></div>
                        <label class="maintenance-switch"><input type="checkbox" name="is_recurring" value="1" data-field="is_recurring"><span aria-hidden="true"></span><strong>{{ __('maintenance.recurring') }}</strong></label>
                            ...
                            @foreach($conditionItems as $condition)
                                <div class="maintenance-condition">
                                    ...
                                    <div class="maintenance-condition-grid">
                                        <label class="maintenance-condition-due">{{ __('maintenance.next_due') }}<div class="maintenance-input-suffix"><input class="form-control" type="{{ $condition['type'] }}" min="0" step="{{ $condition['type'] === 'number' ? '0.01' : '' }}" name="{{ $condition['due'] }}" data-field="{{ $condition['due'] }}">@if($condition['type'] === 'number')<span>{{ $condition['unit'] }}</span>@endif</div></label>
                                        <label>{{ __('maintenance.reminder') }}<div class="maintenance-input-suffix"><input class="form-control" type="number" min="0" name="{{ $condition['reminder'] }}" value="0" data-field="{{ $condition['reminder'] }}"><span>{{ $condition['unit'] }}</span></div></label>
                                        <label>{{ __('maintenance.interval') }}<div class="maintenance-input-suffix"><input class="form-control" type="number" min="1" name="{{ $condition['interval'] }}" data-field="{{ $condition['interval'] }}"><span>{{ $condition['unit'] }}</span></div></label>
                                    </div>
                                </div>
                            @endforeach
                        </div>
...
<div class="modal fade users-modal" id="completeMaintenanceModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered users-modal-dialog"><div class="modal-content"><form method="POST" data-complete-form enctype="multipart/form-data" data-loading-form>@csrf @method('PATCH')<div class="modal-header"><div class="form-modal-heading"><span class="form-modal-heading-icon"><i class="fa-solid fa-circle-check"></i></span><h2 class="modal-title" data-complete-title>{{ __('maintenance.complete') }}</h2></div><button class="btn-close" type="button" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="users-form-grid">
<div><label class="form-label">{{ __('maintenance.performed_at') }} *</label><input class="form-control" type="date" name="performed_at" value="{{ now()->toDateString() }}" required></div><div><label class="form-label">{{ __('maintenance.garage') }}</label><select class="form-select" name="garage_id" data-searchable-database data-search-placeholder="{{ __('ui.search_options') }}" data-no-results="{{ __('ui.no_option_match') }}" data-option-icon="fa-screwdriver-wrench"><option value="">-</option>@foreach($garages as $garage)<option value="{{ $garage->id }}">{{ $garage->name }}</option>@endforeach</select></div>
<div><label class="form-label">{{ __('maintenance.actual_odometer') }}</label><input class="form-control" type="number" min="0" step="0.01" name="odometer_km"></div><div><label class="form-label">{{ __('maintenance.actual_engine_hours') }}</label><input class="form-control" type="number" min="0" step="0.01" name="engine_hours"></div><div><label class="form-label">{{ __('maintenance.completion_cost') }}</label><input class="form-control" type="number" min="0" step="0.01" name="actual_cost"></div><div><label class="form-label">{{ __('maintenance.documents') }}</label><input class="form-control" type="file" name="documents[]" multiple></div><div class="users-form-full"><label class="form-label">{{ __('maintenance.notes') }}</label><textarea class="form-control" name="notes" rows="4"></textarea></div>
</div></div><div class="modal-footer"><button type="button" class="btn users-cancel-button" data-bs-dismiss="modal">{{ __('maintenance.cancel') }}</button><button class="btn btn-primary">{{ __('maintenance.complete') }}</button></div></form></div></div></div>
