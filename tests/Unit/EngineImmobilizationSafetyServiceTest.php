...
test('accepts three stopped samples even when parked telemetry is sparse', function () {
    Carbon::setTestNow('2026-09-03 12:00:00');
    $device = Device::factory()->online()->create(['last_seen_at' => now(), 'last_ignition' => false]);

    foreach ([360, 180, 0] as $secondsAgo) {
    ...
    expect($result['safe'])->toBeTrue()
        ->and($result['sample_count'])->toBe(3)
        ->and($result['window_seconds'])->toBe(360);
});
...
test('decodes the p4 engine bit and fails closed for a running engine', function (int $p4Flags, bool $safe) {
    Carbon::setTestNow('2026-09-03 12:00:00');
    $device = Device::factory()->online()->create(['last_seen_at' => now(), 'last_ignition' => false]);
    stoppedTelemetryWindow($device);

    Position::query()->latest('id')->firstOrFail()->update([
        'raw_data' => [
            'payload' => [
                'io' => ['24' => 0, '85' => 0, '179' => 0, '180' => 0, '239' => 0, '240' => 0, '517' => $p4Flags],
                'obd' => ['speed' => 0, 'rpm' => 0],
                'can' => ['engine_running' => $p4Flags],
            ],
        ],
    ]);

    expect(app(EngineImmobilizationSafetyService::class)->evaluate($device)['safe'])->toBe($safe);
})->with([
    'engine stopped, unrelated P4 flags set' => [317, true],
    'engine running bit set' => [2048, false],
]);
